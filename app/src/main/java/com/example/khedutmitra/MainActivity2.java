...
public class MainActivity2 extends AppCompatActivity implements PopupMenu.OnMenuItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        client.getLastLocation().addOnSuccessListener(MainActivity2.this, new OnSuccessListener<Location>() {
            @Override
    ...
    private void WeatherByCityName(String city){
        ...
        okhttp3.Request request=new okhttp3.Request.Builder()
                .url("https://api.openweathermap.org/data/2.5/forecast?q="+city+"&appid="+BuildConfig.API_KEY+"&units=metric")
                .get().build();
    ...
    private void WeatherByLatLon(String lat,String lon){
        OkHttpClient client=new OkHttpClient();
        okhttp3.Request request=new okhttp3.Request.Builder()
                .url("https://api.openweathermap.org/data/2.5/forecast?lat="+lat+"&lon="+lon+"&appid="+BuildConfig.API_KEY+"&units=metric")
                .get().build();
        StrictMode.ThreadPolicy policy=new StrictMode.ThreadPolicy.Builder().permitAll().build();
        StrictMode.setThreadPolicy(policy);
        try {
    ...
    public boolean onMenuItemClick(MenuItem item) {
        switch (item.getItemId()){
            ...
            case R.id.id_otherCity:
                Intent intent=new Intent(MainActivity2.this,CitySearch.class);
